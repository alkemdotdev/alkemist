...
```sh
npm create alkemist@beta -- my-lab --provider custom
cd my-lab
npm install
npm run verify
```
...
Edit the identity and navigation in `src/lib/site.ts`, then replace the starter content. Keep the generated `package-lock.json`; source-generated sites also retain their `vendor/` directory. The custom provider option does not invent an upload command or assume an account.
