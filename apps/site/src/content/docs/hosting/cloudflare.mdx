...
This is currently a public source-preview workflow: the packages are not published on npm. Clone the Alkemist repository, then with Node 24 run these commands from its root:
...
The generated directory is an independent Astro application. Its Alkemist packages are packed into `vendor/`; they do not depend on an npm release or the continued presence of the source checkout. Keep those tarballs and the generated `package-lock.json` in your repository.
