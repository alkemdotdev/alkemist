...
The source repository is public. The `1.0.0-beta.1` package set is prepared, but registry publishing awaits the first npm publication; until then, the source generator installs local tarballs. The registry generator will create a new project without owning it afterward, while the source generator remains the supported tarball update path. The demo fixtures are synthetic and labeled as such. Cloudflare hosts this demo; GitLab and custom-provider guides are setup recipes, not claims of a completed deployment on your account.
