...
`Slides` gives a document two views: a scrolling Markdown page and a browser presentation. Read is the default on every screen. Both views use the same content and mounted figures, so edits and local parameter changes carry across. The presentation reflows the document for the screen; it does not summarize or duplicate it.
...
Try [One document, two views](/blog/one-document-two-views/). A custom content route can use the same compiler with `present: true` and wrap its rendered `<Content />` in `<Slides title={entry.data.title}>`. Include any route-owned title or cover inside a `section` with `data-alk-slide` if it should appear in Present. Add `present: z.boolean().default(false)` to the collection schema. The integration still requires `slides: true`.
...
Read and Present share the same widget instances. Every screen defaults to Read. Speaker view is available for standalone decks; embedded decks keep independent navigation within their host page. The reading column uses `--alk-document-width` (760px by default).
