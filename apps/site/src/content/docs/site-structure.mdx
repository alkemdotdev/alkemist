...
The default primary navigation is **Blog · Labs · Info**. The Alkemist demo adds a **More ›** disclosure with Docs and Test page. On small screens, one hamburger menu contains both groups and keeps the header on a single row. These menus support keyboard activation, Escape to close and return focus, and dismissal by clicking outside. Navigation remains usable without JavaScript.
...
<AlkLayout
  ...
  moreNavigation={[{ label: 'Docs', href: '/docs/' }]}
>
  <slot />
</AlkLayout>
```

The demo's `src/layouts/SiteLayout.astro` supplies its More and footer links. A site without documentation can use `AlkLayout` directly. Supplying `navigation` replaces the default main links, so unused sections need not appear either.
