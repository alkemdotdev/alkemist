...
**Source preview:** the Alkemist GitHub repository is public, but the packages are not published on npm. Clone the source repository to use the generator; there is no `npx create-alkemist` command yet.

<StartUsing heading="Choose how to start." />

## Create a separate site

Use Node **24.20.0**, matching the repository's `.nvmrc`. Clone the repository, then generate into a new, empty directory:
