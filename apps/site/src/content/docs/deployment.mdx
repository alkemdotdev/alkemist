---
title: Alkemist demo hosting
description: Maintainer reference for this demo's Cloudflare configuration.
order: 90
---

This page describes the Alkemist project's own infrastructure. For your new website, use the [Cloudflare](/docs/hosting/cloudflare/), [GitLab Pages](/docs/hosting/gitlab-pages/), or [custom hosting](/docs/hosting/custom/) guide with your own repository and account.

The site's production address is [alkemist.alkem.dev](https://alkemist.alkem.dev). Cloudflare Pages builds the site from the `alkemdotdev/alkemist` GitHub repository.
...
The root `functions/test/[[asset]].js` Pages Function supplies byte-range responses for four small teaching fixtures. `apps/site/public/_routes.json` limits invocation to those files; the rest of the site remains static. Native Git builds publish the function alongside the site. The deployment check verifies actual partial-response bytes for both audio and video, since Cloudflare Pages static files do not currently honor range requests. This fixture adapter is not a large-media storage service.
