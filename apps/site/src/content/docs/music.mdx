...
Press Play, choose a track, and change its voice. Click a note to edit its pitch, start, length, and velocity. The piano roll is the same reusable `Midi` component that you can embed in an article, tutorial, or music lab.
...
The horizontal axis uses quarter-note beats; the vertical axis uses MIDI pitches. Select a track to work on its notes. Add notes in the grid, drag to move them, and use the selected-note fields for precise edits. Snap controls the rhythmic grid. Duplicate, Delete, Undo, Redo, and Reset provide a short editing loop.

Arrow keys move a selected note in pitch or time. Delete removes it, and Control/Command-Z undoes an edit. The numeric fields provide an alternative to dragging. The grid scrolls inside its container on narrow screens.
...
| ---------- | ---------- | ---------------------------------------------------------------------------- |
| `title`    | Empty      | Visible and accessible player name.                                          |
