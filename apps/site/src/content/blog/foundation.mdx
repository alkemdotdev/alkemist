...
The example application lives in `apps/site`. Its routes and articles consume `@alkemdotdev/alkemist-components`, with `@alkemdotdev/alkemist-theme` supplying the visual system and `@alkemdotdev/alkemist-astro` supplying integration defaults.
...
We chose `Alk` for public components and exported types. `Chart`, `Model`, and `Theorem` are short enough to write repeatedly and specific enough to distinguish them from local application code. The [naming guide](/docs/naming/) records the convention.

## Let existing engines do their job

A good charting interface needs more than line charts. The intended `Chart` vocabulary includes bars, pies, donuts, statistical views, and composition. Vega-Lite supplies a broad rendering grammar; Alkemist will supply file handling, typed presets, consistent figure controls, accessible alternatives, and documentation. The [chart design](/docs/charts/) records the intended API without presenting it as shipped.

## Publish the process

This post is MDX, rendered inside `Layout` with the same theme used by the handbook and homepage. As features land, development articles will include actual demos, diagrams, annotated screenshots, data files, and validation evidence.
