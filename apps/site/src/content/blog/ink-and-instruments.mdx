...
| ------------------------------- | ---------------------------- | ----------------------------------------------------- |
| `Math` or dollar-delimited math | KaTeX typesetting            | Shared fonts/options, MathML, build errors            |
| `Code` or a code fence          | Shiki tokenization           | Inks, captions, line highlights, copy                 |
| `Chart` with a CSV path         | Vega-Lite and Vega rendering | Presets, grids, tables, lazy loading, theme updates   |
| `Model` with a GLB path         | Three.js rendering           | Orbit controls, camera presets, poster, cleanup       |
| `Shader`                        | WebGL2 fragment shading      | A bounded interactive study, controls, pause behavior |

Public component names are plain. Heavy clients load when their figure approaches the viewport. Math and code are rendered at build time. Model and shader animation starts only when requested and stops doing work when offscreen.
