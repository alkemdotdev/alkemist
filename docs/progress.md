...
## Shared Audio and Video components — 2026-09-13

Added extensionless Audio and Video exports in the existing components package. Shared escaped markup and lazy Media Chrome controls serve both Astro output and live playgrounds. The Content category includes the new components; browser-native specimens remain in the HTML/media reference with clear links to the enhanced players. A new media reference covers imports, all props, WebVTT captions/subtitles/chapters, transcripts, download behavior, and supported file playback. Media Chrome and Plyr informed the controls; the synthetic thirty-second wave/tone fixtures are generated locally with FFmpeg.

Controls include play/pause, buffered seeking, skip offsets, volume/mute, selectable rates, loop, caption languages, chapter navigation, and platform fullscreen/PiP/AirPlay. Dense inline settings rows avoid clipping in short audio players, and video controls sit below the full picture. Optional plain-text transcripts and original downloads sit below the controls. Audio can show active caption cues. Styling uses scoped token fallbacks; native controls remain until successful enhancement and work without JavaScript. Removed players release listeners/observers and stop playback. Media and track errors are visible; failed platform PiP requests are handled without disrupting playback.

Observed checks: `npm run verify` passed, including 94 generated output pages and internal links/assets. Packed components compiled in an independent host with Astro and MDX, prop imports, and no Alkemist Layout. Runtime-config/escaping and inherited-prop playground coverage tests passed. Browser checks at desktop and 390px covered actual playback, speed selection, chapter seeking, caption language switching, keyboard seeking, loop, missing-source recovery, live source generation, no-JavaScript native controls, and dark/light presentation without page overflow. The embedded browser advertises PiP but rejects it; the player now reports that limitation and hides the unusable button while playback continues. Persistent fullscreen and AirPlay require a supporting browser/device outside this embedded preview; they are not claimed as physically validated here.

Branch-preview validation found that native Cloudflare Pages returned whole-file `200` responses to media byte-range requests: local chapter seeking worked, but preview seeks reset to zero. Added a narrowly routed Pages Function for the four small generated media fixtures, retaining the native Git deployment system. Deployment verification now checks `206`, Content-Range, and exact audio/video bytes, in addition to build identity and routes. Adopting-site documentation states the server requirement explicitly; large media belongs on range-capable storage/CDNs.

Preview and production browser checks confirmed audio seeks to 10 seconds and video to 20 seconds. The custom-domain CDN advertises Accept-Ranges on full-response metadata but may omit that advisory header on its partial MP4 responses. The delivery gate now checks HEAD metadata plus exact partial bytes separately, following RFC 9110 section 14.3; no player or delivery fallback was required.

## Music examples and MIDI editing — 2026-09-13

Added the extensionless Midi component under Visualization: a reusable piano roll, typed note/track model, four original musical studies, and local synthesized piano, supersaw, and bass voices. Tone.js owns synthesis primitives and @tonejs/midi owns file parsing/encoding. Alkemist owns the compact editor, authoring model, preset arrangements, isolated playback clocks, lifecycle, and generated-source integration. No sample downloads or audio input permissions are required.

The editor provides play/pause/stop, position seeking, tempo, volume, measure-aligned looping, per-track mute/voice, note creation/dragging and numeric edits, rhythmic snap, duplicate/delete, single-step drag undo, redo/reset, and MIDI import/export. Playback-only mode hides editing controls. Unsupported MIDI automation, tempo changes, and percussion are reported at import; this is not a DAW project or audio recorder. Sources are bounded and validated. Static inline scores and accessible note lists remain available without JavaScript; URL sources retain a download link.

Observed checks include the full project gate (91 tests and 97 output pages), standalone packed Astro/MDX consumers, desktop and 390px layouts without page overflow, actual audio-context playback across voices, typed pitch/velocity changes, adding a note to the active bass track, one-step drag undo, delete/undo, local MIDI import, and a real exported file re-imported with 32 notes and its changed supersaw voice. Playground edits regenerate sequence source; a missing file shows an explicit load error. Logs are under `.alkemist/midi-*.log`; the browser-export specimen remains in `.alkemist/midi-downloads/`. Synthesis is implemented and playback was exercised; subjective instrument voicing remains adjustable through further listening review.

## Direct piano-roll editing and a compact toolbar — 2026-09-13

Replaced the MIDI control rack with one top toolbar and horizontal track tabs. Draw, Select, and Erase tools keep edits on the score: draw a note to length, drag to move, pull the right edge to resize, or right-click to delete. Each gesture is one history entry; D/S/E choose tools and Space toggles playback. Instrument and Mute apply to the active track. More holds secondary playback/file controls; Note details reveals precise numeric edits only on demand. MIDI file dropping is handled locally by the existing bounded importer. FL Studio's documented piano-roll interactions informed the tool vocabulary and direct manipulation.
...
## Browser slides and annotation proposal — 2026-09-14

Added `docs/slides-and-annotations.md` with a source-backed recommendation to retain Astro/MDX and use Reveal.js for presentation behavior. After authoring review, simplified the proposal to top-level Markdown slide separators and ordinary MDX components for Note, Step, and SpeakerNotes. An opt-in built-in import map and figure IDs remove routine imports and wrapper markup. The design retains shared widget activation/sizing, semantic annotation targets, readable output, and static export requirements. Plain Markdown handles text, math, code, tables, images, and footnotes; MDX adds components.

Reviewed Marp's first-party syntax references and added an existing-feature/convention comparison. Direct probes confirmed the installed Astro renderer supports footnotes, tables, task lists, and strikethrough while GitHub alerts remain literal. Prettier normalizes Marp's special reveal-list markers, and MDX rejects HTML comments; the proposal records those integration constraints without claiming new functionality.

This is design work only; no slide runtime or authoring extension was added. `npm run verify` passed, including 99 validated output pages, internal links, assets, and build identity. The log is `.alkemist/slides-proposal-verify.log`. Browser validation remains an implementation gate.

## Markdown-first browser slides — 2026-09-14

Implemented the approved design with Astro/MDX and lazy Reveal.js. The default source folder is `src/content/slides/`, publishing at `/slides/` and `/slides/<id>/` in both the demo and generated starter. Plain Markdown supports slide separators, incremental lists, math, code metadata, images, callouts, Mermaid, local footnotes, and explicit speaker comments. MDX decks automatically import the supported unbound widgets and annotation components. Ordinary articles retain their rules and component imports; callouts and diagrams are shared enhancements when slides are enabled.

`Slides` supplies Read/Present, navigation, fullscreen, presenter notes, and prepared browser printing. `Note`, `Step`, `SpeakerNotes`, and `Diagram` have direct component/type exports. Existing Chart, Model, Shader, Audio, Video, and Midi widgets accept IDs and presentation activity. Inactive slides and hidden fragments defer engines and pause playback; switching views preserves mounted widget state. Embedded decks namespace their IDs and references. Diagram sources stay available if rendering is unavailable. The three test talks cover plain Markdown, scientific figures, and media/MIDI.

Browser validation caught and fixed nested section handling in Reveal, local footnote placement, the Shiki/Mermaid ordering conflict, and fragment activation without geometry changes. Independent review added actual Astro-pipeline coverage for heading/slide-ID collisions and transitive footnotes. Preparing packages also exposed generated caches inside the source starter; the template copier now filters them while retaining `.env.example`, with a packaging regression check.

Observed validation: the full gate passed with 109 tests and 105 validated output pages. Chromium and WebKit exercised footnote links, diagrams, widget keyboard controls, persistent canvas identity, forward/backward steps, synchronized speaker previews, audio/video/MIDI pause, GPU print captures, 390px reading without horizontal overflow, and no-JavaScript content. Two independently installed embedded decks retained unique IDs and valid ARIA references, navigated separately, and left the host URL alone. Packed consumers and starter checks cover explicit imports, built-in imports, integration opt-out, fresh installation, source upgrades, drafts, disabled sections, and custom deployment bases. npm audit reported zero vulnerabilities. Evidence is under `.alkemist/slides-*.log` and `.alkemist/slides-*.png`; repeatable browser checks live in `scripts/check-slides-browser.mjs` and `scripts/check-slides-embedded-browser.mjs`.

This implements browser presentation and prepared print views. Editable PowerPoint/video export, coordinate annotations, saved presenter ink, and audience builds that omit speaker notes remain future work. Print media and nonblank current GPU captures were inspected; no universal paper-pagination or editable-export claim is made. Package versions remain unchanged, with a coordinated Changeset for the next release.

## Responsive presentation workspace — 2026-09-14

Slides now provide a full-window Present view, thumbnail overview, speaker view, keyboard help, laser pointer, blackout, timed advance, current-slide links, fullscreen, and prepared printing. Read and Present retain the same widget instances. Native controls and browser modifier shortcuts keep their own keyboard behavior; dialogs restore focus, and leaving Present restores the surrounding page.

Responsive CSS replaces the fixed scaled stage. Headings, prose, images, quotes, and shared figures infer useful layouts from ordinary Markdown; optional layout comments cover specific arrangements. Text fitting is bounded, with readable scrolling for excessive content. Decks inherit the host theme or choose paper, chalk, or blueprint. Scoped CSS variables, class/style hooks, and optional 16:9 or 4:3 framing provide customization. Four improved example decks have local artwork and thumbnail covers, including deterministic light/dark signal diagrams generated from an editable script. The generated starter includes the same theme/layout frontmatter and an SVG cover.

Observed validation: `npm run verify` passed with 111 tests and 107 validated pages; formatting, fresh packed component consumers, and generated-site installation/upgrade/base-path checks passed. Chromium 153 and WebKit 26.5 exercised actual timer navigation, aspect ratios, custom tokens, native fullscreen where supported, overview/help/focus, notes synchronization, media pause, persistent model state, print captures, mobile reading, and no-JavaScript content. The 76-case layout sweep across 1440, 1280, 1024, and 390px found no horizontal or vertical content overflow in the four examples, with text at least 20px.

Browser evidence identified and fixed several integration errors: injected Astro scripts affected layout inference, overview excerpts included speaker text, split images overlapped, video sizing exceeded tablet space, and teardown could restart a disposed model during pagehide. A lifecycle regression now verifies disposal and pageshow restoration. Evidence is retained under `.alkemist/presentation-*.log` and `.alkemist/presentation-*.png`; repeatable checks are in `scripts/check-presentation-browser.mjs` and `scripts/check-slide-layouts-browser.mjs`. Printed current GPU frames and paper-color charts were inspected; universal paper pagination, editable slide export, and saved presenter ink remain outside this implementation.

The deployed preview also exposed a short-window case at 1280×720: fixed-height figures could overlap their reserved area. Charts now measure an optional CSS height while retaining their authored height outside presentations; plot labels stay at their normal size. Model, shader, and video viewports adapt to stage height, and larger editors scroll inside their allocated figure area. The layout check now verifies figure bounds as well as section overflow across all four viewport sizes.

## Shared themes and live figure focus — 2026-09-15

Pages and presentations now share Neutral, Paper, Chalk, and Blueprint semantic styles. Named style is independent of Light/Dark/System; the site header saves both preferences, and a deck can inherit them or select a local override in Tools. Blueprint's grid is a shared surface token. Presentation controls now keep Read/Present and Tools at the top, with navigation and overview together below the stage.

Shader and Chart expose bounded local parameters through `parameters={true|false|keys}` and a shared descriptor/validation/rendering layer. Ordinary props supply initial values; Reset values restores them. Custom-element `getParameters()`/`setParameters()` and `alk:parameters-change` support embedding. Chart changes reuse loaded rows. The new field presentation, Labs comparison, and component guides demonstrate the same APIs outside and inside slides.

Focus promotes the existing widget into the native Popover top layer, preserving its DOM and live values. It supplies background inertness, Tab containment, Escape, focus return, and cleanup for removed figures or inactive slides. Slides adds Focus to figure headings; ordinary pages use the public `Focus` component. Embedded target IDs are remapped within their deck. Browsers without Popover keep ordinary inline controls.

Observed checks: full verify passed with 124 tests and 117 validated pages; formatting passed. Chromium 153 and WebKit 26.5 exercised parameter changes, focus and return, persistent canvas identity, late/removed targets, all eight style/color combinations, and 390px controls. Main text contrast exceeded 10:1 in each named combination. The six-deck layout matrix passed 108 cases across 1440, 1280, 1024, and 390px with no content overflow and minimum 20px text. Existing slide tests retained print captures, notes, media pause, no-JavaScript reading, and lifecycle behavior. Native presenter checks retained real audience synchronization and wake locks, plus permission-denial and receiver adapter coverage. Packed component consumers and generated starter checks passed; two installed embedded instances resolve Focus locally and retain unique IDs and valid ARIA references.

Evidence: `.alkemist/themes-focus-verify-final.log`, `themes-focus-format-check.log`, `themes-focus-browser.log`, `themes-focus-webkit.log`, `themes-focus-layouts.log`, `themes-focus-slides*.log`, `themes-focus-native*.log`, `themes-focus-packages.log`, `themes-focus-starter.log`, and `themes-focus-embedded.log`. Parameter state remains local to the mounted figure; audience parameter synchronization and physical projector/cast operation are not claimed. Coordinated package Changeset included; branch-preview delivery precedes main promotion.

## Articles, decks, and inspectable figures — 2026-09-15

`Presentation` owns shared presenting controls with a continuous article reading layout. `Deck` exposes authored slide boundaries as responsive surfaces, and `Slide` defines a typed section for Astro compositions. Blog and deck routes remain distinct. The navigation label is Presentations; existing `/slides/` paths, content folders, `Slides` imports, `format: slides`, and `slides: true` integration options remain compatible. New examples use `format: deck` and `presentations: true`.

Charts now expose one Data and export disclosure containing a 25-row paginated table of prepared rows, plotted CSV, SVG, and 2× PNG downloads. Original CSV links remain available. Zoom domains survive parameter/theme/size changes and turning interaction off/on; Reset view explicitly restores the full domain. Generation guards prevent stale exports from changing a newer render. CSV string cells are protected against spreadsheet formula prefixes without changing numeric negatives. SVG and PNG use the documented [Vega View export API](https://vega.github.io/vega/docs/api/view/); disabling interaction uses [Vega-Lite interval options](https://vega.github.io/vega-lite/docs/selection.html).

Models share the parameter contract through `view` and `wireframe`. Validated patches update camera/materials in place and restore authored wireframe settings when disabled. Initial framing uses the measured aspect ratio; later resizes preserve orbit direction and relative zoom. Spin remains a separate explicit playback control. New examples include Look closer, the Articles and decks comparison, an expanded live figures lab, and a development article. Public package, integration, starter, and component guides document the same APIs.

Observed local validation: full verify passed with 142 tests and 121 validated output pages. Chromium and WebKit passed desktop/mobile article/deck transitions, original Markdown/footnotes and link behavior, figure identity/parameters, chart table pagination and downloaded file contents, model pixel changes and rejection invariants, and focus behavior. Both engines passed 124 slide/viewport cases across seven decks at a minimum 20px content font. Shared theme checks covered eight style/color combinations. Independent tarball consumers and starter install/upgrade checks passed; installed typed entries and legacy embedded decks retain distinct layouts, independent navigation, unique IDs, and valid ARIA references. Screenshot review covers reading, presenting, focused data inspection, and mobile layouts. It caught and corrected the focused chart minimum-height conflict; browser coverage now checks that the plot makes room for its data panel. Evidence: `.alkemist/layouts-*.log` and `.alkemist/layouts-*.png`.

The earlier site deployment succeeded; its npm canary verification failed while aggregate registry metadata lagged. Release verification now uses immutable exact-version integrity and targeted dist-tags endpoints. Aggregate metadata is consulted only for the existing legacy bootstrap latest-tag safeguard. This revision is staged for branch-preview verification before production promotion; the new canary workflow must complete before reporting registry recovery. Parameters remain local to the mounted figure; audience parameter sync and physical display/casting operation are not implied.
