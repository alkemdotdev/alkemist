...
`Slides` accepts `title`, optional `description`, `view` (`present` by default), `embedded`, and `id`. It is a component inside a host layout, not a document wrapper. A collection route passes the `Content` returned by Astro's `render(entry)` into its slot. The compiler creates the semantic sections for Markdown authors.
...
The demo contains three authored decks:
