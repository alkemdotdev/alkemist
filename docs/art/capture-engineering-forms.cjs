...
// Run there: playwright-cli -s=alkemist-forms run-code --filename=../docs/art/capture-engineering-forms.cjs
// Start Astro on 4335 first. Copy the four PNGs into apps/site/src/assets/forms-engineering after
...
async (page) => {
  ...
  for (const take of ['orbit', 'strata', 'interference', 'assembly']) {
    ...
    await art
      .locator('canvas')
      .screenshot({
        path: 'engineering-poster-' + take + '.png',
        omitBackground: true,
      });
  }
}
